...
class Reporting:
    ...
    def check_if_the_instance_is_none(instance):
        if instance is None:
            return 0
        return float(instance.decode('utf-8'))

    def can_use_catch_or_not(self, start_date, end_date, standard_id):
        if str(end_date) == str(datetime.today().date()):
            return self._generate_report_data(start_date, end_date, standard_id)
        return self._read_from_catch(start_date, end_date, standard_id)

    @staticmethod
    def convert_date_type(start_date, end_date):
        if isinstance(start_date, datetime):
            start_date = datetime.strptime(str(start_date), "%Y-%m-%d %H:%M:%S").date()
    ...
    def _read_from_catch(self, start_date, end_date, standard_id=None):
        start_date, end_date = self.convert_date_type(start_date, end_date)
        ...
        if standard_id is not None:
            ...
            if time_base_standard == 0 and mileage_base_standard == 0 and self.cnt == 0:
                self.generate_and_catch_data(start_date, end_date, self.car_id)
                self.cnt += 1
                return self.can_use_catch_or_not(start_date=start_date, end_date=end_date, standard_id=standard_id)
    ...
    def generate_and_catch_data(self, start_date, end_date, car_id):
