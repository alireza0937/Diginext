...
class ReportingAPIView(APIView):
    def post(self, request):
        # try:
            ...
            start_date, end_date = reporting_instance.convert_date_type(start_date=start_date, end_date=end_date)

            if reporting_instance.check_car_existence():

                if reporting_instance.company_member_check(token=token):
                    data = reporting_instance.can_use_catch_or_not(start_date=start_date.date(), end_date=end_date.date(),
            ...
            return Response({"message": "Your insert Car ID seems to be inactive and does not have a trip yet"},
